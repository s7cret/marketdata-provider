...
class MockWebSocketSupervisor:
    ...
    def run(
        self,
        updates: Iterable[KlineUpdate],
        *,
        backfill_bars: Iterable[MarketBar] = (),
        reconnect_after: int | None = None,
        queue_maxsize: int | None = None,
    ) -> MockStreamResult:
        processed = open_updates = closed_commits = reconnects = backfilled = (
            dropped
        ) = coalesced = 0
        ...
        for update in updates:
            if reconnect_after is not None and processed == reconnect_after:
                ...
                for b in backfill_bars:
                    if b.is_closed:
                        self.store.commit_closed(
                            b, event_time=b.downloaded_at, received_at=b.downloaded_at
                        )
                        backfilled += 1
                cp = self.store.current.get_checkpoint(
                ...
                self.store.current.update_checkpoint(
                    StreamCheckpoint(
                        exchange=update.exchange,
                        market=update.market,
                        symbol=update.symbol,
                        ...
                        last_event_time=cp.last_event_time if cp else None,
                        last_received_at=cp.last_received_at if cp else None,
                        last_reconnect_at=update.received_at or update.event_time,
                        consecutive_reconnects=(cp.consecutive_reconnects if cp else 0)
                        + 1,
                        status="reconnected",
                        updated_at=update.received_at or update.event_time,
                    )
                )
...
def require_live_stream_enabled() -> None:
    if (
        os.getenv("RUN_MARKETDATA_STREAM_TESTS") != "1"
        and os.getenv("MARKETDATA_ALLOW_STREAM") != "1"
    ):
        raise MDNetworkUnavailable(
            "Live WebSocket streaming is disabled unless RUN_MARKETDATA_STREAM_TESTS=1 or MARKETDATA_ALLOW_STREAM=1; use --mock-events for local deterministic stream ingestion"
        )


def overlap_start(
    last_closed: int | None, timeframe: str, overlap_bars: int
) -> int | None:
