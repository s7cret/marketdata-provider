...
def _get_json(
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if r.status_code in _RATE_LIMIT_STATUSES:
                ...
                time.sleep(
                    float(retry_after) if retry_after else min(2.0, 0.25 * (2**attempt))
                )
                continue
    ...
    raise MDNetworkUnavailable(
        "Binance HTTP request failed",
        details={"error": str(last) if last else "unknown"},
    )
...
def _server_time_ms(
    ...
    try:
        ...
        raise MDInvalidExchangeResponse(
            "Binance server time payload missing serverTime"
        ) from e


def binance_get_bars_sync(
    symbol: str,
    timeframe: str,
    start: int | None,
    end: int | None,
    ...
    with httpx.Client(
        ...
        server_time = _server_time_ms(client, base, ns.market, max_retries=max_retries)
        while True:
            ...
            payload = _get_json(
                client, base + endpoint, params, max_retries=max_retries
            )
            if not isinstance(payload, list):
                raise MDInvalidExchangeResponse("Binance kline payload must be a list")
            page = normalize_binance_klines(
                payload,
                symbol=ns.exchange_symbol,
                market=ns.market,
                timeframe=timeframe,
            ...
            if end is not None and cursor >= end:
                break
    # De-duplicate page overlaps without erasing MarketBar finality/provenance.
...
async def binance_get_intrabar_bars(
    symbol: str,
    chart_bar: Bar,
