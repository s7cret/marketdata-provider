...
BINANCE_AGG_TRADES_ENDPOINTS = {
    "spot": "/api/v3/aggTrades",
    "usdm": "/fapi/v1/aggTrades",
}
    ...
    for row in payload:
        try:
            ...
            raise MDInvalidExchangeResponse(
                "Binance aggTrade row is invalid", details={"row": row}
            ) from exc
        ...
        try:
            ...
            if response.status_code in _RATE_LIMIT_STATUSES:
                if attempt >= max_retries:
                    raise MDNetworkUnavailable(
                        "Binance aggTrades rate limit exceeded",
                        details={"status": response.status_code, "params": params},
                    )
            ...
            if attempt >= max_retries:
                raise MDNetworkUnavailable(
                    ...
                    details={"error": str(exc), "params": params},
                ) from exc
            time.sleep(min(2.0, 0.25 * (2**attempt)))
    ...
    *,
    ...
    with httpx.Client(
        ...
        while cursor < end:
            ...
            page = normalize_binance_agg_trades(
                _get_json(client, base + endpoint, params, max_retries=max_retries)
            )
