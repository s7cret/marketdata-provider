...
BINANCE_ENDPOINTS = {"spot": "/api/v3/klines", "usdm": "/fapi/v1/klines"}

def normalize_binance_klines(rows: Sequence[Sequence[Any]], *, symbol: str, market: str, timeframe: str, server_time_ms: int | None = None, include_open_candle: bool = False) -> list[MarketBar]:
    bars: list[MarketBar] = []
    for r in rows:
        ...
        bars.append(MarketBar(
            ...
            trades_count=int(r[8]) if len(r) > 8 and r[8] not in (None, "") else None,
            taker_buy_base_volume=float(r[9]) if len(r) > 9 and r[9] not in (None, "") else None,
            taker_buy_quote_volume=float(r[10]) if len(r) > 10 and r[10] not in (None, "") else None,
        ))
...
class OfflineBinanceRestAdapter:
    def __init__(self, rows: Sequence[Sequence[Any]], *, config: BinanceConfig | None = None, server_time_ms: int | None = None):
        self.rows = rows; self.config = config or BinanceConfig(); self.server_time_ms = server_time_ms
    def get_klines(self, *, symbol: str, market: str, interval: str, start: int | None, end: int | None, limit: int | None = None, include_open_candle: bool = False) -> list[MarketBar]:
        ...
        bars = normalize_binance_klines(rows, symbol=symbol, market=market, timeframe=tf, server_time_ms=self.server_time_ms, include_open_candle=include_open_candle)
