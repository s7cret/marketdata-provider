...
def write_cache_segment(
    ...
    *,
    ...
    seg = cache_segment_dir(
        root, exchange=exchange, market=market, symbol=symbol, timeframe=timeframe
    )
    ...
    (seg / "metadata.json").write_text(
        json.dumps(asdict(meta), sort_keys=True, indent=2) + "\n"
    )
    return meta


def read_cache_segment(
    root: str | Path,
    *,
    exchange: str,
    market: str,
    ...
    bars = [
        Bar(
            ...
            int(r["time_close"]) if r.get("time_close") else None,
        )
        for r in rows
    ]
    validate_bars(bars)
    ...
    if meta.get("runtime_contract_version") != RUNTIME_CONTRACT_VERSION:
        ...
        raise MDInvalidExchangeResponse(
            f"Unsupported cache runtime_contract_version: {meta.get('runtime_contract_version')}"
        )
    ...
    if actual != meta.get("checksum"):
        ...
        raise MDInvalidExchangeResponse(
            ...
            details={"expected": meta.get("checksum"), "actual": actual},
        )
    out = [
        b
        for b in bars
