...
class SegmentStore:
    """Finalized immutable-candle store with manifest/index foundation.

    Stage C keeps CSV segments as the guaranteed dependency-free format while exposing
    the same manifest/index semantics expected for future Parquet partitions.
    """
    ...
    def _init_index(self) -> None:
        with sqlite3.connect(self.index_path) as db:
            ...
            db.execute(
                ...
                "checksum TEXT NOT NULL, downloaded_at INTEGER NOT NULL, "
    ...
    def _paths(self, **key: str) -> tuple[Path, Path]:
        d = self._dir(**key)
        return d / "bars.csv", d / "manifest.json"
    ...
    def read_all(self, *, exchange: str, market: str, symbol: str, timeframe: str, source_kind: str = "trade_kline", start: int | None = None, end: int | None = None) -> list[MarketBar]:
        data_path, manifest_path = self._paths(exchange=exchange, market=market, symbol=symbol, timeframe=timeframe, source_kind=source_kind)
    ...
    def replace_all(self, bars: list[MarketBar], *, exchange: str, market: str, symbol: str, timeframe: str, source_kind: str = "trade_kline") -> SegmentManifest:
        ...
        with sqlite3.connect(self.index_path) as db:
            ...
            if bars:
                db.execute(
                    "INSERT OR REPLACE INTO marketdata_segments(exchange,market,symbol,timeframe,start_time,end_time,rows_count,source_transport,source_kind,checksum,downloaded_at) "
                    "VALUES(?,?,?,?,?,?,?,?,?,?,?)",
                    (manifest.exchange, manifest.market, manifest.symbol, manifest.timeframe, manifest.start_time, manifest.end_time, manifest.rows_count, manifest.source_transport, manifest.source_kind, manifest.checksum, max((b.downloaded_at or 0) for b in bars)),
                )
        return manifest
    ...
    def _row_to_bar(self, r: dict[str, str]) -> MarketBar:
        ...
        return MarketBar(
            ...
            source_transport=r.get("source_transport") or "ws", source_kind=r.get("source_kind") or "trade_kline", is_closed=(r.get("is_closed", "True") in {"True", "true", "1", "1.0"}), downloaded_at=opt_int("downloaded_at"),
        )
