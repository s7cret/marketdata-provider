...
def test_service_fetch_and_materialize_tail_append_and_unsupported(
    ...
    monkeypatch.setattr(
        MarketDataService,
        "_fetch_from_sources",
        lambda self, q: [mb(120_000, close=3.0)] if q.start_ms == 120_000 else [],
    )
...
def test_service_derived_materialize_no_rows_and_month_coverage(
    ...
    service = MarketDataService(cfg)
    monkeypatch.setattr(MarketDataService, "_fetch_from_sources", lambda self, q: [])
...
def test_factories_store_provider_and_live_adapter(
    ...
    class FakeService:
        ...
        def fetch_bars(self, query: BarQuery):
            return series
