...
def test_public_spot_get_bars_success_for_all_exchanges(
    ...
    payloads = {
        ...
        "htx": {
            "data": [
                {"id": 0, "open": 1, "high": 2, "low": 0.5, "close": 1.5, "amount": 10}
            ]
        },
    ...
    _patch_client(
        monkeypatch, [FakeResponse(200, payload) for payload in payloads.values()]
    )
...
def test_public_spot_http_retry_and_error_paths(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    with pytest.raises(MDNetworkUnavailable, match="rate limit"):
        ps._http_get_json(
            "https://x", params={}, timeout=1, user_agent="ua", max_retries=1
        )
    ...
    with pytest.raises(MDNetworkUnavailable, match="HTTP request failed"):
        ps._http_get_json(
            "https://x", params={}, timeout=1, user_agent="ua", max_retries=0
        )
...
def test_public_symbol_normalizers_cover_all_disabled_and_fallback_branches(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payloads = {
        "okx": {
        ...
        "kraken": {
            "result": {
                "bad": {"altname": "XBTUSD", "status": "disabled"},
                "good": {"altname": "ETHUSD", "wsname": "ETH/USD"},
            }
        },
        "kucoin": {
            "data": [
        ...
        "htx": {
            "data": [
                {"sc": "btcusdt", "bcdn": "btc", "qcdn": "usdt", "state": "offline"},
                {"sc": "ethusdt", "bcdn": "eth", "qcdn": "usdt", "state": "online"},
            ]
        },
        "mexc": {
            "symbols": [
        ...
        "other": [
            {"symbol": "BTCUSDT", "base": "", "quote": "USDT"},
            {"symbol": "ETHUSDT", "base": "ETH", "quote": ""},
    ...
    assert (
        normalize_public_spot_symbols(
            "other", payloads["other"], stable_quotes_only=False
        )
        == []
    )
    ...
    monkeypatch.setattr(
        symbols_mod, "_provider_market", lambda exchange, market: "spot"
    )
...
def test_query_first_public_symbol_search_avoids_bulk_catalog(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import marketdata_provider.symbols as symbols_mod
    ...
    assert [
        item.symbol for item in search_symbols("coinbase", "spot", query="BTC", limit=1)
    ] == ["BTC-USDT"]
    assert [
        item.symbol for item in search_symbols("kraken", "spot", query="BTC", limit=1)
    ] == ["XBTUSDT"]
    assert [
        item.symbol for item in search_symbols("gateio", "spot", query="BTC", limit=1)
    ] == ["BTC_USDT"]
    assert [
        item.symbol for item in search_symbols("htx", "spot", query="BTC", limit=1)
    ] == ["BTCUSDT"]

    assert all(
        "/products" not in url or url.endswith("BTC-USDT") for url, _ in calls[:1]
    )
    assert any(params == {"pair": "XBTUSDT"} for _, params in calls)
    assert search_symbols("coinbase", "spot", query="", limit=1) == []


def test_query_first_public_symbol_search_skips_failed_candidates(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import marketdata_provider.symbols as symbols_mod
...
def test_query_first_helper_edge_branches() -> None:
    ...
    assert (
        _normalize_query_first_payload(
            "htx", "btcusdt", "BTC", "USDT", {"status": "bad"}
        )
        == []
    )
