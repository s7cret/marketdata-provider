...
def test_distribution_zip_builder_and_hygiene(tmp_path: Path) -> None:
    output = tmp_path / "marketdata-provider-4.0.2.zip"
    build_zip(Path.cwd(), output, archive_root="marketdata-provider-4.0.2")
    with zipfile.ZipFile(output) as zf:
        names = zf.namelist()
    assert "marketdata-provider-4.0.2/pyproject.toml" in names
