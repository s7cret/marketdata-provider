...
def test_search_symbols_supports_native_derivatives_without_network(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_http_get_json(
        ...
        *,
        ...
        if "okx.com" in url:
            ...
            if inst_type == "MARGIN":
                return {"data": [{"instId": "BTC-USDT", "baseCcy": "BTC", "quoteCcy": "USDT", "state": "live"}]}
            if inst_type == "SWAP":
                return {"data": [
                    {"instId": "BTC-USDT-SWAP", "instFamily": "BTC-USDT", "baseCcy": "", "quoteCcy": "", "settleCcy": "USDT", "ctType": "linear", "state": "live"},
                    {"instId": "BTC-USD-SWAP", "instFamily": "BTC-USD", "baseCcy": "", "quoteCcy": "", "settleCcy": "BTC", "ctType": "inverse", "state": "live"},
                ]}
            if inst_type == "FUTURES":
                return {"data": [{"instId": "BTC-USDT-260626", "instFamily": "BTC-USDT", "baseCcy": "", "quoteCcy": "", "settleCcy": "USDT", "ctType": "linear", "state": "live"}]}
        if "futures.kraken.com" in url:
            return {"result": "success", "instruments": [
                {"symbol": "PF_XBTUSD", "type": "flexible_futures", "tradeable": True, "base": "BTC", "quote": "USD"},
                {"symbol": "PI_XBTUSD", "type": "futures_inverse", "tradeable": True, "base": "BTC", "quote": "USD"},
                {"symbol": "FF_XBTUSD_260626", "type": "flexible_futures", "tradeable": True, "base": "BTC", "quote": "USD"},
            ]}
        if "api-futures.kucoin.com" in url:
            return {"data": [
                {"symbol": "XBTUSDTM", "baseCurrency": "XBT", "quoteCurrency": "USDT", "settleCurrency": "USDT", "isInverse": False, "status": "Open", "expireDate": None},
                {"symbol": "XBTUSDM", "baseCurrency": "XBT", "quoteCurrency": "USD", "settleCurrency": "XBT", "isInverse": True, "status": "Open", "expireDate": None},
                {"symbol": "XBTMM26", "baseCurrency": "XBT", "quoteCurrency": "USD", "settleCurrency": "XBT", "isInverse": True, "status": "Open", "expireDate": 1782432000000},
            ]}
        if "api.kucoin.com/api/v3/margin/symbols" in url:
            return {"data": [{"symbol": "BTC-USDT", "baseCurrency": "BTC", "quoteCurrency": "USDT", "enableTrading": True}]}
        if "api.bitget.com/api/v2/margin/currencies" in url:
            return {"data": [{"symbol": "BTCUSDT", "baseCoin": "BTC", "quoteCoin": "USDT", "status": "1"}]}
        if "api.bitget.com/api/v2/mix/market/contracts" in url:
            product = params.get("productType")
            if product == "USDT-FUTURES":
                return {"data": [{"symbol": "BTCUSDT", "baseCoin": "BTC", "quoteCoin": "USDT", "symbolType": "perpetual", "status": "normal"}]}
            if product == "COIN-FUTURES":
                return {"data": [{"symbol": "BTCUSD", "baseCoin": "BTC", "quoteCoin": "USD", "symbolType": "perpetual", "status": "normal"}]}
        if "gateio.ws/api/v4/margin/currency_pairs" in url:
            return [{"id": "BTC_USDT", "base": "BTC", "quote": "USDT", "trade_status": "tradable"}]
        ...
        if "gateio.ws/api/v4/delivery/usdt/contracts" in url:
            return [{"name": "BTC_USDT_20260925", "type": "direct", "in_delisting": False}]
        if "linear-swap-api" in url:
            business_type = params.get("business_type")
            if business_type == "swap":
                return {"status": "ok", "data": [{"contract_code": "BTC-USDT", "symbol": "BTC", "trade_partition": "USDT", "contract_status": 1}]}
            return {"status": "ok", "data": [{"contract_code": "BTC-USDT-260626", "symbol": "BTC", "trade_partition": "USDT", "contract_status": 1}]}
        if "swap-api" in url:
            return {"status": "ok", "data": [{"contract_code": "BTC-USD", "symbol": "BTC", "contract_status": 1}]}
        if "contract.mexc.com/api/v1/contract/detail" in url:
            return {"success": True, "data": [
                {"symbol": "BTC_USDT", "baseCoin": "BTC", "quoteCoin": "USDT", "settleCoin": "USDT", "state": 0, "apiAllowed": True, "automaticDelivery": 0},
                {"symbol": "BTC_USD", "baseCoin": "BTC", "quoteCoin": "USD", "settleCoin": "BTC", "state": 0, "apiAllowed": True, "automaticDelivery": 0},
            ]}
...
def test_public_market_get_bars_maps_derivative_endpoints(monkeypatch: pytest.MonkeyPatch) -> None:
    from marketdata_provider.exchanges import public_spot as ps

    calls: list[tuple[str, dict[str, object]]] = []

    def fake_get_json(url: str, *, params: dict[str, object], timeout: float, user_agent: str, max_retries: int) -> Any:
        del timeout, user_agent, max_retries
        calls.append((url, params))
        if "okx.com" in url:
            return {"data": [["0", "1", "2", "0.5", "1.5", "10"]]}
        if "futures.kraken.com" in url:
            return {"candles": [{"time": 0, "open": "1", "high": "2", "low": "0.5", "close": "1.5", "volume": "10"}]}
        if "api-futures.kucoin.com" in url:
        ...
        if "hbdm.com" in url:
            return {"status": "ok", "data": [{"id": 0, "open": 1, "high": 2, "low": 0.5, "close": 1.5, "amount": 10}]}
        if "contract.mexc.com" in url:
            return {"success": True, "data": {"time": [0], "open": [1], "high": [2], "low": [0.5], "close": [1.5], "vol": [10]}}
        raise AssertionError((url, params))
...
def test_market_data_service_routes_public_native_markets(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(service_mod, "public_market_get_bars_sync", fake_public_market_get_bars_sync, raising=False)
    ...
    query = BarQuery(InstrumentKey("okx", "delivery", "BTC-USD-SWAP"), parse_timeframe("1m"), 0, 60_000)
