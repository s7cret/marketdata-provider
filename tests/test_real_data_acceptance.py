...
def test_real_binance_btcusdt_1d_derived_from_1m_matches_official(tmp_path):
    ...
    derived = MarketDataService(
        MarketDataConfig(storage=StorageConfig(cache_dir=tmp_path / "derived"), history=HistoryConfig(enabled=True, base_timeframe="1m"))
    ).fetch_bars(query)
    official = binance_get_bars_sync("BTCUSDT", "1D", start, end, MarketDataConfig().binance, market="spot")
    ...
    for left, right in zip(derived.bars, official, strict=True):
        assert (left.time, left.open, left.high, left.low, left.close) == (right.time, right.open, right.high, right.low, right.close)
...
def test_real_binance_btcusdt_15m_archive_rest_coverage_has_no_gaps(tmp_path):
    ...
    query = BarQuery(InstrumentKey("binance", "spot", "BTCUSDT"), parse_timeframe("15m"), start, end, gap_policy="fail")

    series = MarketDataService(MarketDataConfig(storage=StorageConfig(cache_dir=tmp_path))).fetch_bars(query)
